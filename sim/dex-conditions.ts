...
export interface EventMethods {
	...
	onTryEatItem?:
		| boolean
		| ((this: Battle, item: Item, pokemon: Pokemon) => boolean | void);
	onTryHeal?: (
		this: Battle,
		relayVar: number,
		target: Pokemon,
	...
	onSourceLockMove?:
		| string
		| ((this: Battle, pokemon: Pokemon) => void | string);
	...
	onSourceAccuracyMissPriority?: number;
...
export interface PokemonConditionData
	extends Partial<Condition>, PokemonEventMethods {}
export interface SideConditionData
	extends
		Partial<Omit<Condition, "onStart" | "onRestart" | "onEnd">>,
		SideEventMethods {}
		...
		>
{
	...
	constructor(data: AnyObject) {
		super(data);
		this.effectType = ["Weather", "Status", "Terrain"].includes(
			data.effectType,
		)
			? data.effectType
			: "Condition";
...
const EMPTY_CONDITION: Condition = Utils.deepFreeze(
	new Condition({ name: "", exists: false }),
);
...
export class DexConditions {
	...
	get(name?: string | Effect | null): Condition {
		...
		return this.getByID(
			name.startsWith("item:") || name.startsWith("ability:")
				? (name as ID)
				: toID(name),
		);
	}
